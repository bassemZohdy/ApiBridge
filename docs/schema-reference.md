...
| `enableResponseCache` | boolean | `false` | `true` \| `false` | When `true`, caches GET proxy responses in-process using Caffeine. Cache key = full upstream URL + query string. TTL = `CACHE_TTL_SECONDS` (default 60s). Max entries = `CACHE_MAX_SIZE` (default 1000, LRU eviction). Non-GET requests (PUT/POST/DELETE/PATCH) call `invalidateAll()` for consistency. No additional infrastructure required. |
